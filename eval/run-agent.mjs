...
export function fuzzDiscoverTask({ prepPath, repoDir, pluginDir, draftPath }) {
  return [
    ...
    ` - daemon: do NOT hand-roll the server plumbing (it lost the crash last time). Emit a`,
    `   { "driver":"daemon-protocol", "protocol":"resp"|"inline", "buildCommand", "serverCommand"`,
    `   (with {PORT}), "readyProbe", "sequence":[[args]...] } discovery — 1-3 setup ops from`,
    `   \`vocabulary\` then the crafted command with an over-limit COUNT. The framework driver builds,`,
    `   starts the ASan server, drives the sequence, and captures the abort to stdout for you.`,
