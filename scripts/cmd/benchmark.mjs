...
function listCases() {
  ...
  // Only *recorded* corpus cases — a directory with both ground truth (expected.json)
  // and a recorded run (findings.json) — are scored by the regression net. Fixtures
  // that ship source + expected.json but no recorded findings.json belong to the live
  // eval harness (bench/run.mjs); scoring them here would fail on "record a run first".
  return readdirSync(CASES_DIR)
    .filter((n) => statSync(join(CASES_DIR, n)).isDirectory())
    .filter((n) => existsSync(join(CASES_DIR, n, "expected.json")) && existsSync(join(CASES_DIR, n, "findings.json")));
}
