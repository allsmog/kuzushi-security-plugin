// kuzushi starter (Joern/CPG): user input -> LDAP search filter sink (CWE-90).
...
val sinks = cpg.call.code(
  "(?i)(\\.search\\(|ldap.*search|InitialDirContext|DirContext|NamingEnumeration|ldapsearch|searchFilter|ldap_search)"
).l
val flows = sinks.reachableByFlows(sources).take(25).l
System.err.println(s"[kuzushi-starter] CWE-90: ${sources.size} sources, ${sinks.size} sinks, ${flows.size} flows")
flows.foreach { p =>
  val els = p.elements
  if (els.nonEmpty) {
    ...
    println(s"""{"cwe":"CWE-90","filePath":"$file","sourceLine":${src.lineNumber.getOrElse(0)},"sinkLine":${snk.lineNumber.getOrElse(0)}}""")
  }
}
}
