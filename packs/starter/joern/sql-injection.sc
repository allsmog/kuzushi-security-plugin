// kuzushi starter (Joern/CPG): user input -> SQL query sink (CWE-89).
...
val sinks = cpg.call.code(
  "(?i)(\\.query\\(|\\.execute\\(|executeQuery|executeUpdate|rawQuery|createQuery|prepareStatement|cursor\\.execute|knex\\.raw|sequelize\\.query|db\\.exec)"
).l
val flows = sinks.reachableByFlows(sources).take(25).l
System.err.println(s"[kuzushi-starter] CWE-89: ${sources.size} sources, ${sinks.size} sinks, ${flows.size} flows")
flows.foreach { p =>
  val els = p.elements
  if (els.nonEmpty) {
    ...
    println(s"""{"cwe":"CWE-89","filePath":"$file","sourceLine":${src.lineNumber.getOrElse(0)},"sinkLine":${snk.lineNumber.getOrElse(0)}}""")
  }
}
